...
// cm4 is a small conservative-update count-min sketch implementation with 4-bit
// counters.
type cm4 struct {
	s    [depth]nvec
	mask uint32
...
func newCM4(w int) *cm4 {
	...
	// use 4 counters per item per level, for a total of 16 counters or 8 bytes
	// per item, matching the TinyLFU paper.
...
func (c *cm4) add(keyh uint64) {
	// The loop unrolling prevents this function from being inlined, but it still
	// results in a slight overall speedup.
...
func (c *cm4) counterOffset(keyh uint64, level int) uint32 {
	// counterOffset gets inlined and the compiler removes the duplicated
	// computations of h1 and h2, so there is no benefit to accepting h1 and h2 as
	// arguments.
...
func (c *cm4) reset() {
	// There is no point in unrolling this loop, the cost is dominated by
	// nvec.reset, which is O(n).
	for _, n := range c.s {
		n.reset()
	}
}
...
func newNvec(w int) nvec { return make(nvec, w/2) }
